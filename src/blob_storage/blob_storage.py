...
Lives in its own top-level package, `blob_storage`, shared by both
src/training/ and src/inference/ (and by extension both RunPod workers under
src/runpod/, which COPY it in at build time) — not nested inside either one,
and not named "storage" living inside "training", because importing a
package literally called "training" from inference code (or vice versa)
would be a confusing, misleading dependency for something that's really just
a generic storage client.
"""
