#!/usr/bin/env node
/**
 * Cancels a job submitted via submit_training_job.js or
 * submit_inference_job.js, using POST /cancel/{job_id}.
 * https://docs.runpod.io/serverless/endpoints/operation-reference
 *
 * Stops an in-progress job (tears down the worker) or removes a queued job
 * before it starts, and returns immediately with the canceled status —
 * RunPod doesn't wait for teardown to finish before responding.
 *
 * Shared between both workers on purpose, same reasoning as
 * check_job_status.js sitting alongside it in src/runpod/scripts/: it's just
 * a POST against RunPod's generic cancel endpoint, nothing training- or
 * inference-specific about it. Which credentials it uses is never inferred
 * or defaulted — you say --endpoint train or --endpoint infer explicitly,
 * same "explicit over implicit" rule as checkpoint_s3_key having no default
 * in src/inference/config.py.
 *
 * Usage:
 *   node kill_job.js <job_id> --endpoint train|infer
 */
...
// script actually accepts. Runnable via `npm run kill:train-job -- <job_id>`
// / `npm run kill:infer-job -- <job_id>`, or directly with
// `node kill_job.js <job_id> --endpoint train|infer`.
const USAGE = `Usage: node kill_job.js <job_id> --endpoint train|infer
...
Example:
  npm run kill:train-job -- abc123
  npm run kill:infer-job -- abc123`
...
const killJob = async () => {
  const url = `https://api.runpod.ai/v2/${RUNPOD_ENDPOINT_ID}/cancel/${jobId}`
...
if (require.main === module) {
  killJob().catch((error) => {
    console.error('Failed to cancel job:', error.response ? error.response.data : error.message)
    process.exit(1)
  })
}
