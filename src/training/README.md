...
| `sed`-patching `train.py` to change the resume iteration | `train.py` never gets hand-edited; resume behavior is a config field (`resume_from`, `override_iteration`) | Never mutate vendored/third-party source in place — patch through config or a thin wrapper. |
...
| No retry/failure handling — a crashed Colab cell just... stops | Checkpointing on every eval interval + automatic resume-from-last-checkpoint on restart | Idempotency and retries under a budget — this is verbatim brightwheel JD language. Currently config-level (`resume.*` in `src/configs/train.yaml`); a job-queue-enforced retry budget like the durable-job-queue interview-prep exercise you built separately is the natural next step once this is running. |
